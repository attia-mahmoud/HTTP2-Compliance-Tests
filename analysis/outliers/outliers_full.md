...
Total outliers found: 125

## Outliers for Apache-2.4.62
...
| 135 | A field value MUST NOT start with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). | 500 | unmodified |
| 136 | A field value MUST NOT end with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). | 500 | unmodified |
...
| 104 | Promised requests MUST be safe (see Section 9.2.1 of [HTTP]) and cacheable (see Section 9.2.3 of [HTTP]). | unmodified | dropped |
...
| 119 | An endpoint that receives a SETTINGS frame with any unknown or unsupported identifier MUST ignore that setting. | unmodified | dropped |
...
| 155 | Unsupported settings MUST be ignored. | unmodified | dropped |
...
| Test ID | Description | Outlier Behavior | Common Behavior |
|---------|-------------|------------------|----------------|
| 166 | HTTP/2 does not support the 101 (Switching Protocols) informational status code (Section 15.2.2 of [HTTP]). | reset | 500 |
