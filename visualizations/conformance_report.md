...
| Nginx | 31 (37.3%) | 46 (55.4%) | 83 |
| Lighttpd | 33 (39.8%) | 44 (53.0%) | 83 |
...
| Nghttpx | 110 | ignore | unmodified | A reserved 1-bit field. The semantics of this bit are undefined, and the bit MUST remain unset (0x00) when sending and MUST be ignored when receiving. (server side) |
...
| Nghttpx | 151 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| HAproxy | 110 | ignore | unmodified | A reserved 1-bit field. The semantics of this bit are undefined, and the bit MUST remain unset (0x00) when sending and MUST be ignored when receiving. (server side) |
...
| HAproxy | 126 | error | modified | Trailers MUST NOT include pseudo-header fields (Section 8.3). |
| HAproxy | 135 | error | unmodified | A field value MUST NOT start with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
...
| HAproxy | 151 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| HAproxy | 165 | error | modified | Pseudo-header fields MUST NOT appear in a trailer section. |
...
| Caddy | 110 | ignore | unmodified | A reserved 1-bit field. The semantics of this bit are undefined, and the bit MUST remain unset (0x00) when sending and MUST be ignored when receiving. (server side) |
...
| Caddy | 139 | error | modified | An endpoint MUST NOT generate an HTTP/2 message containing keep-alive header field (RFC9113 Section 8.2.2) |
...
| Caddy | 141 | error | modified | An endpoint MUST NOT generate an HTTP/2 message containing upgrade header field (RFC9113 Section 8.2.2) |
...
| Caddy | 151 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| Node | 110 | ignore | unmodified | A reserved 1-bit field. The semantics of this bit are undefined, and the bit MUST remain unset (0x00) when sending and MUST be ignored when receiving. (server side) |
...
| Node | 128 | error | modified | Field names MUST NOT contain control characters (0x00-0x1F) |
| Node | 129 | error | modified | Field names MUST NOT contain ASCII SP (0x20) |
| Node | 130 | error | dropped | Field names MUST NOT contain DEL character (0x7F) |
| Node | 131 | error | modified | Field names MUST NOT contain high byte characters (0x80-0xFF) |
| Node | 132 | error | modified | With the exception of pseudo-header fields (Section 8.3), which have a name that starts with a single colon, field names MUST NOT include a colon (ASCII COLON, 0x3a). |
| Node | 133 | error | modified | A field value MUST NOT contain line feed (ASCII LF, 0x0a). (Tested at the start of the value) |
| Node | 134 | error | modified | A field value MUST NOT contain carriage return (ASCII CR, 0x0d). (Tested at the start of the value) |
| Node | 135 | error | modified | A field value MUST NOT start with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
| Node | 136 | error | modified | A field value MUST NOT end with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
| Node | 144 | error | dropped | An endpoint MUST NOT send frames other than PRIORITY on a closed stream. |
| Node | 151 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| Envoy | 110 | ignore | unmodified | A reserved 1-bit field. The semantics of this bit are undefined, and the bit MUST remain unset (0x00) when sending and MUST be ignored when receiving. (server side) |
...
| Envoy | 135 | error | unmodified | A field value MUST NOT start with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
| Envoy | 136 | error | unmodified | A field value MUST NOT end with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
...
| Envoy | 151 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| Cloudflare | 4 | error | modified | Client preface must include a SETTINGS frame |
...
| Cloudflare | 8 | error | unmodified | Values greater than 16,384 MUST NOT be sent unless receiver has set larger SETTINGS_MAX_FRAME_SIZE |
...
| Cloudflare | 87 | error | modified | A SETTINGS frame MUST be sent by both endpoints at the start of a connection and MAY be sent at any other time by either endpoint over the lifetime of the connection. (Tested from the client side.) |
...
| Cloudflare | 110 | ignore | unmodified | A reserved 1-bit field. The semantics of this bit are undefined, and the bit MUST remain unset (0x00) when sending and MUST be ignored when receiving. (server side) |
...
| Cloudflare | 126 | error | modified | Trailers MUST NOT include pseudo-header fields (Section 8.3). |
...
| Cloudflare | 151 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| Mitmproxy | 1 | error | unmodified | Receiving any frame other than HEADERS or PRIORITY on a stream in this (idle) state MUST be treated as a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 3 | error | unmodified | the connection preface starts with the string: PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n |
| Mitmproxy | 4 | error | modified | Client preface must include a SETTINGS frame |
| Mitmproxy | 5 | error | unmodified | If this stream (initially in the idle state) is initiated by the server, as described in Section 5.1.1, then receiving a HEADERS frame MUST also be treated as a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 6 | error | unmodified | An endpoint MUST NOT send any type of frame other than HEADERS, RST_STREAM, or PRIORITY in the reserved (local) state. |
| Mitmproxy | 7 | error | unmodified | If an endpoint receives additional frames, other than WINDOW_UPDATE, PRIORITY, or RST_STREAM, for a stream that is in the half-closed (remote) state, it MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. |
| Mitmproxy | 8 | error | unmodified | Values greater than 16,384 MUST NOT be sent unless receiver has set larger SETTINGS_MAX_FRAME_SIZE |
| Mitmproxy | 10 | ignore | unmodified | A reserved 1-bit field. The semantics of this bit are undefined, and the bit MUST remain unset (0x00) when sending and MUST be ignored when receiving. |
| Mitmproxy | 11 | error | unmodified | Streams initiated by a client MUST use odd-numbered stream identifiers. |
| Mitmproxy | 12 | error | unmodified | Streams initiated by a server MUST use even-numbered stream identifiers. |
| Mitmproxy | 13 | error | unmodified | The identifier of a newly established stream MUST be numerically greater than all streams that the initiating endpoint has opened or reserved. |
| Mitmproxy | 14 | error | dropped | An endpoint MUST NOT send a RST_STREAM in response to a RST_STREAM frame |
| Mitmproxy | 15 | error | unmodified | If a PRIORITY frame is received with a stream identifier of 0x00, the recipient MUST respond with a connection error of type PROTOCOL_ERROR |
...
| Mitmproxy | 20 | error | unmodified | The stream identifier for a SETTINGS frame MUST be zero (0x00). |
| Mitmproxy | 21 | error | unmodified | A SETTINGS frame with a length other than a multiple of 6 octets MUST be treated as a connection error (Section 5.4.1) of type FRAME_SIZE_ERROR. |
| Mitmproxy | 22 | error | unmodified | The initial value of SETTINGS_ENABLE_PUSH is 1. Any value other than 0 or 1 MUST be treated as a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 23 | error | unmodified | A server MUST NOT explicitly set this value (SETTINGS_ENABLE_PUSH) to 1. A server MAY choose to omit this setting (SETTINGS_ENABLE_PUSH) when it sends a SETTINGS frame, but if a server does include a value, it MUST be 0. |
| Mitmproxy | 24 | error | unmodified | A server MUST NOT send a PUSH_PROMISE frame if it receives the SETTINGS_ENABLE_PUSH (0x02) parameter set to a value of 0. |
| Mitmproxy | 25 | error | unmodified | For SETTINGS_INITIAL_WINDOW_SIZE, values above the maximum flow-control window size of 2^31-1 (2147483647) MUST be treated as a connection error (Section 5.4.1) of type FLOW_CONTROL_ERROR. |
| Mitmproxy | 26 | error | unmodified | The value advertised by an endpoint MUST be between initial value (2^14 = 16,384) and maximum allowed frame size (2^24-1 = 16,777,215 octets), inclusive. |
| Mitmproxy | 27 | ignore | unmodified | An endpoint that receives a SETTINGS frame with any unknown or unsupported identifier MUST ignore that setting. |
| Mitmproxy | 28 | error | unmodified | If the Stream Identifier field of a PUSH_PROMISE frame specifies the value 0x00, a recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 29 | error | unmodified | The promised stream identifier MUST be a valid choice for the next stream sent by the sender (see 'new stream identifier' in Section 5.1.1). (Using Odd-numbered Stream ID (Invalid for Server)) |
| Mitmproxy | 30 | error | unmodified | If a PING frame is received with a Stream Identifier field value other than 0x00, the recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 31 | error | unmodified | With the CONNECT method, the " :scheme" and " :path" pseudo-header fields MUST be omitted. (Tested with only :path present) |
| Mitmproxy | 32 | error | unmodified | With the CONNECT method, the " :scheme" and " :path" pseudo-header fields MUST be omitted. (Tested with only :scheme present) |
...
| Mitmproxy | 35 | error | unmodified | An endpoint MUST treat a change to SETTINGS_INITIAL_WINDOW_SIZE that causes any flow-control window to exceed the maximum size as a connection error (Section 5.4.1) of type FLOW_CONTROL_ERROR. |
| Mitmproxy | 36 | error | unmodified | If a CONTINUATION frame is received with a Stream Identifier field of 0x00, the recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 37 | error | unmodified | A CONTINUATION frame MUST be preceded by a HEADERS, PUSH_PROMISE or CONTINUATION frame without the END_HEADERS flag set. (Using HEADERS frame with END_HEADERS flag set) |
| Mitmproxy | 38 | error | unmodified | A CONTINUATION frame MUST be preceded by a HEADERS, PUSH_PROMISE or CONTINUATION frame without the END_HEADERS flag set. (Using PUSH_PROMISE frame with END_HEADERS flag set) |
| Mitmproxy | 39 | error | unmodified | The header fields in PUSH_PROMISE and any subsequent CONTINUATION frames MUST be a valid and complete set of request header fields. |
| Mitmproxy | 40 | error | unmodified | Trailers MUST NOT include pseudo-header fields (Section 8.3). |
| Mitmproxy | 41 | error | unmodified | Field names MUST be converted to lowercase when constructing an HTTP/2 message. |
| Mitmproxy | 42 | error | unmodified | Field names MUST NOT contain control characters (0x00-0x1F) |
| Mitmproxy | 43 | error | unmodified | Field names MUST NOT contain ASCII SP (0x20) |
| Mitmproxy | 44 | error | unmodified | Field names MUST NOT contain DEL character (0x7F) |
| Mitmproxy | 45 | error | unmodified | Field names MUST NOT contain high byte characters (0x80-0xFF) |
| Mitmproxy | 46 | error | unmodified | With the exception of pseudo-header fields (Section 8.3), which have a name that starts with a single colon, field names MUST NOT include a colon (ASCII COLON, 0x3a). |
| Mitmproxy | 47 | error | unmodified | A field value MUST NOT contain line feed (ASCII LF, 0x0a). (Tested at the start of the value) |
| Mitmproxy | 48 | error | unmodified | A field value MUST NOT contain carriage return (ASCII CR, 0x0d). (Tested at the start of the value) |
| Mitmproxy | 49 | error | unmodified | A field value MUST NOT start with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
| Mitmproxy | 50 | error | unmodified | A field value MUST NOT end with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
| Mitmproxy | 51 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing connection header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 52 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing proxy-connection header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 53 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing keep-alive header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 54 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing transfer-encoding header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 55 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing upgrade header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 56 | error | unmodified | The TE header field MAY be present in an HTTP/2 request; when it is, it MUST NOT contain any value other than 'trailers'. |
| Mitmproxy | 57 | error | unmodified | Pseudo-header fields are not HTTP header fields. Endpoints MUST NOT generate pseudo-header fields other than those defined in this document. |
| Mitmproxy | 58 | error | unmodified | Pseudo-header fields defined for requests MUST NOT appear in responses. |
| Mitmproxy | 59 | error | unmodified | Pseudo-header fields defined for responses MUST NOT appear in requests. |
| Mitmproxy | 60 | error | unmodified | All pseudo-header fields sent from a client MUST appear in a field block before all regular field lines. Any request or response that contains a pseudo-header field that appears in a field block after a regular field line MUST be treated as malformed (Section 8.1.1). |
| Mitmproxy | 61 | error | unmodified | All pseudo-header fields sent from a server MUST appear in a field block before all regular field lines. Any request or response that contains a pseudo-header field that appears in a field block after a regular field line MUST be treated as malformed (Section 8.1.1). |
| Mitmproxy | 62 | error | unmodified | The same pseudo-header field name MUST NOT appear more than once in a field block. A field block for an HTTP request or response that contains a repeated pseudo-header field name MUST be treated as malformed (Section 8.1.1). Tested with a request frame with the same value. |
| Mitmproxy | 63 | error | unmodified | The same pseudo-header field name MUST NOT appear more than once in a field block. A field block for an HTTP request or response that contains a repeated pseudo-header field name MUST be treated as malformed (Section 8.1.1). Tested with a request frame with different values. |
| Mitmproxy | 64 | error | unmodified | The same pseudo-header field name MUST NOT appear more than once in a field block. A field block for an HTTP request or response that contains a repeated pseudo-header field name MUST be treated as malformed (Section 8.1.1). Tested with a response frame with the same value. |
| Mitmproxy | 65 | error | unmodified | The same pseudo-header field name MUST NOT appear more than once in a field block. A field block for an HTTP request or response that contains a repeated pseudo-header field name MUST be treated as malformed (Section 8.1.1). Tested with a response frame with different values. |
| Mitmproxy | 66 | error | unmodified | Clients MUST NOT generate a request with a Host header field that differs from the ":authority" pseudo-header field. |
| Mitmproxy | 67 | error | unmodified | ':authority' MUST NOT include the deprecated userinfo subcomponent for "http" or "https" schemed URIs. |
| Mitmproxy | 68 | error | unmodified | The ":path" pseudo-header field MUST NOT be empty for "http" or "https" URIs; "http" or "https" URIs that do not contain a path component MUST include a value of '/'. |
| Mitmproxy | 69 | error | unmodified | All HTTP/2 requests MUST include exactly one valid value for the ":method", ":scheme", and ":path" pseudo-header fields, unless they are CONNECT requests (Section 8.5). An HTTP request that omits mandatory pseudo-header fields is malformed (Section 8.1.1). (Tested with :method missing) |
| Mitmproxy | 70 | error | unmodified | All HTTP/2 requests MUST include exactly one valid value for the ":method", ":scheme", and ":path" pseudo-header fields, unless they are CONNECT requests (Section 8.5). An HTTP request that omits mandatory pseudo-header fields is malformed (Section 8.1.1). (Tested with :scheme missing) |
| Mitmproxy | 71 | error | unmodified | For HTTP/2 responses, a single ":status" pseudo-header field is defined that carries the HTTP status code field. This pseudo-header field MUST be included in all responses, including interim responses; otherwise, the response is malformed. |
| Mitmproxy | 72 | error | unmodified | A client cannot push. Thus, servers MUST treat the receipt of a PUSH_PROMISE frame as a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 73 | error | unmodified | In the PUSH_PROMISE frame, the server MUST include a method in the ":method" pseudo-header field that is safe and cacheable. |
| Mitmproxy | 74 | error | unmodified | With the CONNECT method, the ":scheme" and ":path" pseudo-header fields MUST be omitted. (Tested with both present) |
| Mitmproxy | 75 | ignore | unmodified | With the CONNECT method, The ":authority" pseudo-header field contains the host and port to connect to |
...
| Mitmproxy | 80 | error | unmodified | DATA frames MUST be associated with a stream. |
| Mitmproxy | 81 | error | unmodified | If a DATA frame is received whose Stream Identifier field is 0x00, the recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 82 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the idle state.) |
| Mitmproxy | 83 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| Mitmproxy | 85 | error | unmodified | A HEADERS frame without the END_HEADERS flag set MUST be followed by a CONTINUATION frame for the same stream. |
| Mitmproxy | 86 | error | unmodified | If a HEADERS frame is received whose Stream Identifier field is 0x00, the recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 87 | error | modified | A SETTINGS frame MUST be sent by both endpoints at the start of a connection and MAY be sent at any other time by either endpoint over the lifetime of the connection. (Tested from the client side.) |
| Mitmproxy | 88 | ignore | unmodified | Unsupported settings MUST be ignored. |
| Mitmproxy | 89 | error | unmodified | The promised stream identifier MUST be a valid choice for the next stream sent by the sender (see 'new stream identifier' in Section 5.1.1). (Using Already Used Stream ID) |
| Mitmproxy | 90 | error | unmodified | The promised stream identifier MUST be a valid choice for the next stream sent by the sender (see 'new stream identifier' in Section 5.1.1). (Using Lower Stream ID) |
| Mitmproxy | 91 | error | unmodified | A PUSH_PROMISE frame without the END_HEADERS flag set MUST be followed by a CONTINUATION frame for the same stream. |
| Mitmproxy | 92 | error | unmodified | PUSH_PROMISE MUST NOT be sent if the SETTINGS_ENABLE_PUSH setting of the peer endpoint is set to 0. |
...
| Mitmproxy | 94 | error | unmodified | CONTINUATION frames MUST be associated with a stream. |
| Mitmproxy | 95 | error | unmodified | If the END_HEADERS flag is not set, this frame MUST be followed by another CONTINUATION frame. A receiver MUST treat the receipt of any other type of frame or a frame on a different stream as a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 96 | error | unmodified | Other frames (from any stream) MUST NOT occur between the HEADERS frame and any CONTINUATION frames that might follow. |
| Mitmproxy | 97 | error | unmodified | An endpoint that receives a HEADERS frame without the END_STREAM flag set after receiving the HEADERS frame that opens a request or after receiving a final (non-informational) status code MUST treat the corresponding request or response as malformed (Section 8.1.1). |
| Mitmproxy | 98 | error | unmodified | A field value MUST NOT contain line feed (ASCII LF, 0x0a). (Tested at the middle of the value) |
| Mitmproxy | 99 | error | unmodified | A field value MUST NOT contain line feed (ASCII LF, 0x0a). (Tested at the end of the value) |
| Mitmproxy | 100 | error | unmodified | A field value MUST NOT contain carriage return (ASCII CR, 0x0d). (Tested at the middle of the value) |
| Mitmproxy | 101 | error | unmodified | A field value MUST NOT contain carriage return (ASCII CR, 0x0d). (Tested at the end of the value) |
| Mitmproxy | 102 | error | unmodified | Pseudo-header fields MUST NOT appear in a trailer section. |
| Mitmproxy | 103 | error | unmodified | All HTTP/2 requests MUST include exactly one valid value for the ":method", ":scheme", and ":path" pseudo-header fields, unless they are CONNECT requests (Section 8.5). An HTTP request that omits mandatory pseudo-header fields is malformed (Section 8.1.1). (Tested with :path missing) |
| Mitmproxy | 104 | error | unmodified | Promised requests MUST be safe (see Section 9.2.1 of [HTTP]) and cacheable (see Section 9.2.3 of [HTTP]). |
| Mitmproxy | 105 | error | unmodified | PUSH_PROMISE frames MUST NOT be sent by the client. |
| Mitmproxy | 106 | error | unmodified | Receiving any frame other than HEADERS or PRIORITY on a stream in this (idle) state MUST be treated as a connection error (Section 5.4.1) of type PROTOCOL_ERROR. (server side) |
| Mitmproxy | 107 | error | unmodified | If an endpoint receives additional frames, other than WINDOW_UPDATE, PRIORITY, or RST_STREAM, for a stream that is in the half-closed (remote) state, it MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (server side) |
| Mitmproxy | 108 | error | unmodified | Values greater than 16,384 MUST NOT be sent unless receiver has set larger SETTINGS_MAX_FRAME_SIZE. (server side) |
| Mitmproxy | 110 | ignore | unmodified | A reserved 1-bit field. The semantics of this bit are undefined, and the bit MUST remain unset (0x00) when sending and MUST be ignored when receiving. (server side) |
| Mitmproxy | 111 | error | unmodified | If a PRIORITY frame is received with a stream identifier of 0x00, the recipient MUST respond with a connection error of type PROTOCOL_ERROR. (server side) |
...
| Mitmproxy | 115 | error | unmodified | The stream identifier for a SETTINGS frame MUST be zero (0x00). |
| Mitmproxy | 116 | error | unmodified | A SETTINGS frame with a length other than a multiple of 6 octets MUST be treated as a connection error (Section 5.4.1) of type FRAME_SIZE_ERROR. |
...
| Mitmproxy | 118 | error | unmodified | The value advertised by an endpoint MUST be between initial value (2^14 = 16,384) and maximum allowed frame size (2^24-1 = 16,777,215 octets), inclusive. |
| Mitmproxy | 119 | ignore | unmodified | An endpoint that receives a SETTINGS frame with any unknown or unsupported identifier MUST ignore that setting. |
| Mitmproxy | 120 | error | unmodified | If a PING frame is received with a Stream Identifier field value other than 0x00, the recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
...
| Mitmproxy | 124 | error | unmodified | If a CONTINUATION frame is received with a Stream Identifier field of 0x00, the recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 125 | error | unmodified | A CONTINUATION frame MUST be preceded by a HEADERS, PUSH_PROMISE or CONTINUATION frame without the END_HEADERS flag set. (Using HEADERS frame with END_HEADERS flag set) |
| Mitmproxy | 126 | error | modified | Trailers MUST NOT include pseudo-header fields (Section 8.3). |
| Mitmproxy | 127 | error | unmodified | Field names MUST be converted to lowercase when constructing an HTTP/2 message. |
| Mitmproxy | 128 | error | unmodified | Field names MUST NOT contain control characters (0x00-0x1F) |
| Mitmproxy | 129 | error | unmodified | Field names MUST NOT contain ASCII SP (0x20) |
| Mitmproxy | 130 | error | unmodified | Field names MUST NOT contain DEL character (0x7F) |
| Mitmproxy | 131 | error | unmodified | Field names MUST NOT contain high byte characters (0x80-0xFF) |
| Mitmproxy | 132 | error | unmodified | With the exception of pseudo-header fields (Section 8.3), which have a name that starts with a single colon, field names MUST NOT include a colon (ASCII COLON, 0x3a). |
| Mitmproxy | 133 | error | unmodified | A field value MUST NOT contain line feed (ASCII LF, 0x0a). (Tested at the start of the value) |
| Mitmproxy | 134 | error | unmodified | A field value MUST NOT contain carriage return (ASCII CR, 0x0d). (Tested at the start of the value) |
| Mitmproxy | 135 | error | unmodified | A field value MUST NOT start with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
| Mitmproxy | 136 | error | unmodified | A field value MUST NOT end with an ASCII whitespace character (ASCII SP or HTAB, 0x20 or 0x09). |
| Mitmproxy | 137 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing connection header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 138 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing proxy-connection header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 139 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing keep-alive header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 140 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing transfer-encoding header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 141 | error | unmodified | An endpoint MUST NOT generate an HTTP/2 message containing upgrade header field (RFC9113 Section 8.2.2) |
| Mitmproxy | 142 | error | unmodified | The TE header field MAY be present in an HTTP/2 request; when it is, it MUST NOT contain any value other than 'trailers'. |
| Mitmproxy | 143 | error | unmodified | Pseudo-header fields are not HTTP header fields. Endpoints MUST NOT generate pseudo-header fields other than those defined in this document. |
| Mitmproxy | 144 | error | dropped | An endpoint MUST NOT send frames other than PRIORITY on a closed stream. |
| Mitmproxy | 148 | error | unmodified | DATA frames MUST be associated with a stream. |
| Mitmproxy | 149 | error | unmodified | If a DATA frame is received whose Stream Identifier field is 0x00, the recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 150 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the idle state.) |
| Mitmproxy | 151 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| Mitmproxy | 153 | error | unmodified | A HEADERS frame without the END_HEADERS flag set MUST be followed by a CONTINUATION frame for the same stream. |
| Mitmproxy | 154 | error | unmodified | If a HEADERS frame is received whose Stream Identifier field is 0x00, the recipient MUST respond with a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 155 | ignore | unmodified | Unsupported settings MUST be ignored. |
...
| Mitmproxy | 157 | error | unmodified | CONTINUATION frames MUST be associated with a stream. |
| Mitmproxy | 158 | error | unmodified | If the END_HEADERS flag is not set, this frame MUST be followed by another CONTINUATION frame. A receiver MUST treat the receipt of any other type of frame or a frame on a different stream as a connection error (Section 5.4.1) of type PROTOCOL_ERROR. |
| Mitmproxy | 159 | error | unmodified | Other frames (from any stream) MUST NOT occur between the HEADERS frame and any CONTINUATION frames that might follow. |
| Mitmproxy | 160 | error | unmodified | An endpoint that receives a HEADERS frame without the END_STREAM flag set after receiving the HEADERS frame that opens a request or after receiving a final (non-informational) status code MUST treat the corresponding request or response as malformed (Section 8.1.1). |
| Mitmproxy | 161 | error | unmodified | A field value MUST NOT contain line feed (ASCII LF, 0x0a). (Tested at the middle of the value) |
| Mitmproxy | 162 | error | unmodified | A field value MUST NOT contain line feed (ASCII LF, 0x0a). (Tested at the end of the value) |
| Mitmproxy | 163 | error | unmodified | A field value MUST NOT contain carriage return (ASCII CR, 0x0d). (Tested at the middle of the value) |
| Mitmproxy | 164 | error | unmodified | A field value MUST NOT contain carriage return (ASCII CR, 0x0d). (Tested at the end of the value) |
| Mitmproxy | 165 | error | modified | Pseudo-header fields MUST NOT appear in a trailer section. |
| Mitmproxy | 166 | error | unmodified | HTTP/2 does not support the 101 (Switching Protocols) informational status code (Section 15.2.2 of [HTTP]). |
...
| Azure-AG | 4 | error | modified | Client preface must include a SETTINGS frame |
...
| Azure-AG | 8 | error | unmodified | Values greater than 16,384 MUST NOT be sent unless receiver has set larger SETTINGS_MAX_FRAME_SIZE |
...
| Azure-AG | 87 | error | modified | A SETTINGS frame MUST be sent by both endpoints at the start of a connection and MAY be sent at any other time by either endpoint over the lifetime of the connection. (Tested from the client side.) |
...
| Nginx | 4 | error | modified | Client preface must include a SETTINGS frame |
...
| Nginx | 8 | error | unmodified | Values greater than 16,384 MUST NOT be sent unless receiver has set larger SETTINGS_MAX_FRAME_SIZE |
...
| Nginx | 87 | error | modified | A SETTINGS frame MUST be sent by both endpoints at the start of a connection and MAY be sent at any other time by either endpoint over the lifetime of the connection. (Tested from the client side.) |
...
| Lighttpd | 4 | error | modified | Client preface must include a SETTINGS frame |
| Lighttpd | 7 | error | unmodified | If an endpoint receives additional frames, other than WINDOW_UPDATE, PRIORITY, or RST_STREAM, for a stream that is in the half-closed (remote) state, it MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. |
...
| Lighttpd | 83 | error | unmodified | If a DATA frame is received whose stream is not in the 'open' or 'half-closed (local)' state, the recipient MUST respond with a stream error (Section 5.4.2) of type STREAM_CLOSED. (Tested in the half-closed (remote) state.) |
...
| Lighttpd | 87 | error | modified | A SETTINGS frame MUST be sent by both endpoints at the start of a connection and MAY be sent at any other time by either endpoint over the lifetime of the connection. (Tested from the client side.) |
